 ...
 *
 * This will, in turn, inform players of their turn and wait for them to respond
 * with a move.
 */
public class Game implements Runnable {

	/** This class's Logger. */
	private static final Logger LOG = Logger.getLogger("com.nullprogram.chess.models.Game");
	...
	private final Collection<GameListener> listeners = new CopyOnWriteArraySet<>();
	...
	public final void run() {
		while (Boolean.FALSE.equals(done)) {
			...
			if (Boolean.TRUE.equals(done)) {
				/*
				 * Game may have ended abruptly during the player's potentially lengthy turn.
				 */
				return;
			}
			...
			if (Boolean.TRUE.equals(board.checkmate(opp))) {
				done = true;
				if (opp == Side.BLACK) {
				...
				return;
			} else if (Boolean.TRUE.equals(board.stalemate(opp))) {
				done = true;
				setStatus("Stalemate!");
				winner = null;
				callGameListeners(GameEvent.GAME_END);
